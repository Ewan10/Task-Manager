...
class TaskManager implements Serializable {
    ...
    private LocalDate dateAccess(int w) {			              //The dateAccess() method simply takes an int and returns
      LocalDate date = taskStorage.get(w).getDate();	      //a date from a task object stored in the list of //TaskManager. It is called in the insertionSort method.
      return date;
    }
  
  private void insertionSort () {	                          //insertionSort method sorts the objects according to the //date from nearest to furthest in future. It is called in the
